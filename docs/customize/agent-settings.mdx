...
agent = Agent(
    ...
    enhance_task=True,            # Enable automatic task enhancement
...
- `enhance_task`: Enable/disable automatic task enhancement. Defaults to `True`.
  - When enabled, the agent uses the LLM to clarify task completion criteria before execution
  - Helps resolve ambiguous tasks by adding minimal clarification without expanding scope
  - Example: "Get the report" becomes "Locate and access the report"
  - Disable if you want tasks executed exactly as written without any modification
